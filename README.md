...
**流程**
1. 若目录中存在 `.ass` 文件，询问是否转换为 `.srt`（转换结果命名为 `<文件夹名><集数>.srt`）
2. 按集数范围查找文件名中包含对应集数的 `.srt`（精确数字匹配，不误匹配相邻集数）
3. 生成 CSV

**示例**
```bash
# 文件夹内有 .ass 文件，按提示确认后转换，再生成 CSV
python srt/process.py 暗河传 1 40

# 文件夹内已经是 .srt 文件，直接生成 CSV
python srt/process.py 山河枕 5 10
```

**输出**
- 转换后的 SRT：`暗河传/暗河传01.srt` …（仅在选择转换时生成）
- CSV：`暗河传/暗河传01-40.csv`

> SRT 文件名只要包含对应的集数数字即可被识别，不限定前缀格式。找不到某集时会报错退出。
